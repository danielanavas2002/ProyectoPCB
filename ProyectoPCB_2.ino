...
unsigned long L_timerDuration_P2 = 24UL * 60UL * 60UL;
...
void setup() {
  ...
  Serial.begin(9600);
  // Definir Botones
  pinMode(buttonL, INPUT_PULLUP); //Input Boton de Reset
  pinMode(buttonR, INPUT_PULLUP); //Input Boton de Comida
  pinMode(buttonS, INPUT_PULLUP); //Input Boton de IU
  // LCD
  lcd.init();
  lcd.backlight();
  lcd.clear();
...
void loop() {
  ...
  switch (estadoActual){
    ...
    case s45: // Pastilla 2 Abierto con Alarma
      ...
      delay(500);
      noTone(buzzer);
      servoP.write(180); //Mover Servomotor a 90°
      break;
